...
@Dao
interface SongWithAlbumDao{
    @Transaction
    @Query("SELECT * FROM Song")
    fun getAllSongs(): List<SongWithAlbum>

    @Transaction
    @Query("SELECT * FROM Song where albumName = :albumName")
    fun getSongsByAlbumName(albumName: String): List<SongWithAlbum>
    ...
    fun getSongByName(name:String):SongWithAlbum

    @Transaction
    @Query("select * from Song where trackId = :id")
    fun getSongTrackId(id:Int):SongWithAlbum
}
